...
hide_text <- function(x) {
  if (knitr::is_latex_output()) {
    sprintf("\\textcolor{%s}{%s}", "white", x)
  } else if (knitr::is_html_output()) {
    sprintf("<span style='color: %s;'>%s</span>", "white",
      x)
  } else x
}
```
...
`r hide_text("R implicitly converts them to all be the same type")`
...
**Answer:** (highlight to read) 
`r hide_text("Vectors can be of only one data type. R tries to
...
**Answer:** (highlight to read)
`r hide_text("Only one. There is no memory of past data types, and the coercion
...
**Answer:** (highlight to read) 
`r hide_text("logical → numeric → character ←  logical")`
...
|----------------|-----------------------------|
| Disc           | 𝜋 × r&sup2; ×𝒽         |
| Half-Ellipsoid | 2/3 × 𝜋 × 𝓁/2 × 𝒽/2 × 𝓌/2 |
