...
height of scallop shells) can be stored in the simpler forms, the
...
marine science and examing the relationship between the length and
weight of bivalve shells.
...
collect data on the length, width, and height of shells, use a few
mathematical formula (dare we call it a model!) to make prediction about
...
There are bivalve shells available for you to measure. Select at least
5, then measure the height, width, and length in inches. Feel free to
team up.
...
> Record vectors to store the measurements. (If you have used R before
> and are familiar with the data.frame structure, you are welcome to use
> instead).

When you are done, we need to export the data so that it's easy to turn
around and load into R. There are many functions to help you read data
...
Alas! We have measured in inches but I'm going to ask you next to work
in metric. The unit conversion for inches to cm is 1 inch = 2.54 cm.

#### TASK:

> Convert the measurements from inches into centimeters and store in a
> second set of vectors (or data.frame).
...
Also, calcium carbonate has a density of 1.3 g/cc. Using this fact,
now estimate the mass of each specimen.
